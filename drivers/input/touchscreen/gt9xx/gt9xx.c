...
static const char *goodix_ts_name = "qcom-tpd";
...
static ssize_t gt9xx_gesture_mode_get_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt9xx_gesture_mode_set_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *gt9xx_gesture_mode_proc = NULL;
static const struct file_operations config_gesture_mode_ops = {
    .owner = THIS_MODULE,
    .read = gt9xx_gesture_mode_get_proc,
    .write = gt9xx_gesture_mode_set_proc,
};
#endif
...
int gtp_tpd_usb_plugin(int charger_on)
{
	gtp_FG_charging_status = charger_on;
	GTP_INFO("[GTP] %s, charger plug %s", __func__, (charger_on == 1)?"IN":"OUT");
	return 0;
}
...
void gtp_usb_plugin(bool mode) //add by zhoujinhong
{
    s8 ret = -1;
    s8 retry = 0;
...
void gtp_usb_exit(bool mode)	//add by zhoujinhong
{
    s8 ret = -1;
    s8 retry = 0;
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            GTP_INFO("GTP exit Charge mode!");
        }
        msleep(10);
    }
    GTP_ERROR("GTP send exit Charge cmd failed.");
}
...
static void goodix_ts_work_func(struct work_struct *work)
{
...
#if GTP_CHARGER_EN
	if((gtp_FG_charging_status) && (gtp_charging_flag == 0)) {
		gtp_charging_flag = 1;
		gtp_usb_plugin(true);
	}else if((gtp_FG_charging_status == 0) && (gtp_charging_flag == 1)) {
		gtp_charging_flag = 0;
		gtp_usb_exit( true);
	}
#endif
    ...
    {               
        ...
        if (ret > 0)
        {     
            if ((doze_buf[2] == 'w') || (doze_buf[2] == 's') || (doze_buf[2] == 'e') ||
                (doze_buf[2] == 'c') || (doze_buf[2] == 'v') || (doze_buf[2] == 'z'))
            {
                s32 key_code = 0;
            ...
            else if (doze_buf[2] == 0xBA) //R:0xAA, L:0xBB, D:0xAB, U:0xBA
            {
		#ifdef ENABLE_ALL_LOG
                char *direction[4] = {"Right", "Down", "Up", "Left"};
		#endif
                s32 key_direction[4] = {KEY_RIGHT, KEY_DOWN, GESTURE_UP, KEY_LEFT};
                u8 type = ((doze_buf[2] & 0x0F) - 0x0A) + (((doze_buf[2] >> 4) & 0x0F) - 0x0A) * 2;
                
                GTP_INFO("%s slide to light up the screen!", direction[type]);
                //doze_status = DOZE_WAKEUP;
                input_report_key(ts->input_dev, key_direction[type], 1);
                input_sync(ts->input_dev);
                input_report_key(ts->input_dev, key_direction[type], 0);
                input_sync(ts->input_dev);
                // clear 0x814B
                doze_buf[2] = 0x00;
                gtp_i2c_write(i2c_connect_client, doze_buf, 3);
            ...
            else if (0xCC == doze_buf[2])
            {
                ...
                //doze_status = DOZE_WAKEUP;
                input_report_key(ts->input_dev, GESTURE_DOUBLECLICK, 1);
                input_sync(ts->input_dev);
                input_report_key(ts->input_dev, GESTURE_DOUBLECLICK, 0);
                input_sync(ts->input_dev);
                // clear 0x814B
                doze_buf[2] = 0x00;
                gtp_i2c_write(i2c_connect_client, doze_buf, 3);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 10)
    {
        ...
        if (ret > 0)
        {
            GTP_INFO("GTP wakeup sleep i2c test.\n");
        #if (!GTP_GESTURE_WAKEUP)
            {
                gtp_int_sync(25);
            #if GTP_ESD_PROTECT
...
//#ifdef	GTP_CONFIG_OF
	...
	#if GTP_CHARGER_SWITCH	//copy charger cfg to config_charger !
		ts->gtp_charge_cfg_len = cfg_info_charger_len[sensor_id];
		memset(&config_charger[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
		memcpy(&config_charger[GTP_ADDR_LENGTH], send_charger_cfg_buf[sensor_id], ts->gtp_charge_cfg_len);
	#endif
                        ...
                        const char __user *buffer, size_t count, loff_t *off)
{
    ...
    if(strncmp(buffer, "0", 1) == 0) {
        ...
        ts->gesture_mode_switch = 1;
        GTP_INFO("Set gesture mode ON\n");
    }
...
#if GTP_GESTURE_WAKEUP
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_DOUBLECLICK);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_UP);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_W);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_S);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_E);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_C);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_V);
    input_set_capability(ts->input_dev, EV_KEY, GESTURE_Z);
#endif 
...
u32 gtp_get_charger_status(void)
{
		...
		//add your charger detect code here!
	return gtp_FG_charging_status;
}
#endif
...
static ssize_t switch_fw_version_read(struct switch_dev *sdev, char *buf)
{
    s32 ret = -1;
    u8 cfg_buf[16] = {0};
    u8 fw_buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};

    GTP_DEBUG_FUNC();

    ret = gtp_i2c_read(i2c_connect_client, fw_buf, sizeof(fw_buf));
    if (ret < 0)
    {
        GTP_ERROR("GTP read firmware failed");
        return -1;
    }

    ret = gtp_i2c_read_dbl_check(i2c_connect_client, GTP_REG_CONFIG_DATA, &cfg_buf[0], 1);
    if (ret != SUCCESS)
    {
    ...
    if (buf[5] == 0x00)
    {
        ret = sprintf(buf, "%c%c%c_%02x%02x 0x%x", fw_buf[2], fw_buf[3],
                             fw_buf[4], fw_buf[7], fw_buf[6], cfg_buf[0]);
    }
    else
    {
        ret = sprintf(buf, "%c%c%c%c_%02x%02x 0x%x", fw_buf[2], fw_buf[3],
                    fw_buf[4], fw_buf[5], fw_buf[7], fw_buf[6], cfg_buf[0]);
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
        GTP_ERROR("I2C communication test failed, stop probe!");
        return -ENODEV;
    }
...
#if GTP_GESTURE_WAKEUP
    ts->gesture_mode_switch = 0; //init state: off
    doze_status = DOZE_DISABLED;
#endif
 ...
 //   spin_lock_init(&ts->esd_lock);
...
#if GTP_GESTURE_WAKEUP
    gt9xx_gesture_mode_proc = proc_create(GTP_GESTURE_MODE_PROC_FILE, 0666, NULL,
                                          &config_gesture_mode_ops);
    if (gt9xx_gesture_mode_proc == NULL)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GTP_GESTURE_MODE_PROC_FILE);
    }
    else
    {
        GTP_DEBUG("create proc entry %s success", GTP_GESTURE_MODE_PROC_FILE);
    }
#endif
    ...
    if(ret < 0)
    {
        GTP_ERROR("Register switch device error.");
    }
...
void gtp_esd_switch(struct i2c_client *client, s32 on)
{
    ...
    {
        if(!atomic_cmpxchg(&ts->esd_running, 0, 1)) 
        {
    ...
    {
        if (atomic_cmpxchg(&ts->esd_running, 1, 0))
        {
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    if (ts->enter_update)
    {
        ...
        atomic_cmpxchg(&ts->esd_running, 1, 0);	 
        return;
    }
...
static int __init goodix_ts_init(void)
{
    ...
    //GTP_INFO("GTP driver installing...%d\n",ret);
    return ret; 
}
